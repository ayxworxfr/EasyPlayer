...
import com.evildoer.player.data.source.db.LiteOrmHelper;
...
public class AppRepository implements AppContract {
    ...
    @Override
    public Observable<List<Video>> insert(List<Video> videos) {
        return mLocalDataSource.insert(videos);
    }

    @Override
    public Observable<Video> update(Video video) {
        return mLocalDataSource.update(video);
    }

    @Override
    public Observable<Video> setVideoAsFavorite(Video video, boolean favorite) {
        return mLocalDataSource.setVideoAsFavorite(video, favorite);
    }
}
